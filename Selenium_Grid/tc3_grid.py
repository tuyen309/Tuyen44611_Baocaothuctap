...
class Testcase3(unittest.TestCase):
    ...
    def test_case3(self):
        ...
        time.sleep(1)
        driver.find_element(By.XPATH,'.//select[@id="listings-sort"]/option[contains(string(), "Price (Highest)")]').click()
        time.sleep(5)
        pricelist =[]
        for j in range(1,9):
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        Testcase3.BROWSER = sys.argv.pop()
        Testcase3.PLATFORM = sys.argv.pop()
